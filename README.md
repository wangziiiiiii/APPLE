...
Note: The data provided for testing has undergone primer removal.

### 1) Minimap2 alignment and filtering on the reads(optional)
```
library(APPLE)
minimap2(
  reference = "Homo_sapiens.GRCh38.dna.primary_assembly.fa.gz",
  work_dir = "/home/yeclab01/final/bedFip",
  threads = 16,
  filter_flags = 2308
)
```

### 2) Extracting polyAsite and detect polyA tail
```
Extract_polyAsite(
  work_dir = "/home/yeclab01/final/bedFip",
  intron_max = 50000, min_tail_length = 6,
  sample_size = 10000, bedtools_path = "bedtools",
  remove_temp_files = TRUE
)
```

### 3) Load raw polyA data
```
QpolyA <- Load.PolyA(dir = './')
```
...
### 5) Feature annotation and APA quantification
``` 
QpolyA <- Annotate.PolyA(QpolyA,'Homo_sapiens.GRCh38.113.gtf')
```

### 6) Filter low-confidence PolyA Clusters
```
QpolyA<- Filter.PolyA(QpolyA,min_count = 10,min_sample = 2)
```

### 7) Map tail lengths to PolyA Clusters
```
QpolyA = mapTail(QpolyA)
```

### 8) Statistical analysis of polyA tail length changes
(1) Sample-level tests: t-test, Wilcoxon, and linear mixed models
```
sample_info<- data.frame(
  sample = c("NC-1", "NC-2","Fip1-1", "Fip1-2","Fip2-1","Fip2-2"),
  condition = c("NC", "NC","Fip1", "Fip1","Fip2","Fip2"),
  lib_id = c("lib1", "lib2", "lib3", "lib4", "lib5", "lib6")  # 可选
)
result <- polyAlength(
  QpolyA = QpolyA,
...
### 9) Differential Expression Analysis of Poly(A) Sites
```
